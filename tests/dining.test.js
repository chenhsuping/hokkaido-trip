...
describe('parseDining 排序', () => {
  ...
  it('點心排在午餐後、晚餐前，不會掉進「未列入」那一檔排到最後', () => {
    const out = parseDining([
      { 日期: '2027-01-03', 餐別: '晚餐', '餐廳/地點': 'C' },
      { 日期: '2027-01-03', 餐別: '點心', '餐廳/地點': 'B' },
      { 日期: '2027-01-03', 餐別: '午餐', '餐廳/地點': 'A' },
    ]);
    expect(out.map(d => d.name)).toEqual(['A', 'B', 'C']);
  });
  ...
  it('同餐別多筆時維持試算表原本的順序', () => {
    const out = parseDining([
      { 日期: '2027-01-03', 餐別: '點心', '餐廳/地點': '美瑛選果' },
      { 日期: '2027-01-03', 餐別: '點心', '餐廳/地點': '札幌農學校' },
    ]);
    expect(out.map(d => d.name)).toEqual(['美瑛選果', '札幌農學校']);
  });
